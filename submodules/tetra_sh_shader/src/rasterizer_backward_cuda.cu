...
struct BrickMeta {
    const int64_t* starts;   // [B,3]
    const int64_t* shapes;   // [B,3]
    const int64_t* offsets;  // [B+1]
};

__device__ inline void sample_point_packed(
    const float3& p,
    const float3& vmin,
    const float3& vmax,
    int vx, int vy, int vz,
    ...
    const float* __restrict__ vertex_color,
    const uint8_t* __restrict__ vertex_valid,
    const BrickMeta& meta,
    float& sigma_out,
    float3& color_out,
    VertexWeight (&weights)[8],
    int& wcount) {
    auto clamp01 = [](float v) { return fminf(fmaxf(v, 0.f), 1.f); };
    const int64_t start_x = meta.starts[brick_idx * 3 + 0];
    const int64_t start_y = meta.starts[brick_idx * 3 + 1];
    const int64_t start_z = meta.starts[brick_idx * 3 + 2];
    const int64_t size_x = meta.shapes[brick_idx * 3 + 0];
    const int64_t size_y = meta.shapes[brick_idx * 3 + 1];
    const int64_t size_z = meta.shapes[brick_idx * 3 + 2];
    const int lx0 = static_cast<int>(vx - start_x);
    const int ly0 = static_cast<int>(vy - start_y);
    const int lz0 = static_cast<int>(vz - start_z);
    const int sx = static_cast<int>(size_x + 1);
    const int sy = static_cast<int>(size_y + 1);
    const int sz = static_cast<int>(size_z + 1);
    const int64_t base = meta.offsets[brick_idx];
    ...
    float t[2] = {1.f - uz, uz};
    int ix0 = lx0;
    int ix1 = min(lx0 + 1, sx - 1);
    int iy0 = ly0;
    int iy1 = min(ly0 + 1, sy - 1);
    int iz0 = lz0;
    int iz1 = min(lz0 + 1, sz - 1);
    const int xs[2] = {ix0, ix1};
    const int ys[2] = {iy0, iy1};
    const int zs[2] = {iz0, iz1};
    ...
    for (int a = 0; a < 2; ++a) {
        for (int b = 0; b < 2; ++b) {
            for (int c = 0; c < 2; ++c) {
                ...
                color_out.z += wgt * cz;
                if (wgt > 0.f && wcount < 8) {
                    weights[wcount].idx = static_cast<int>(id);
                    weights[wcount].w = wgt;
                    ++wcount;
                }
            }
...
__device__ inline void compute_segment_values_packed(
    const float3& p0,
    const float3& p1,
    const float3& vmin,
    const float3& vmax,
    ...
    float sigma0, sigma1;
    float3 col0, col1;
    sample_point_packed(p0, vmin, vmax, vx, vy, vz, brick_idx,
                        vertex_sigma, vertex_color, vertex_valid, meta,
                        sigma0, col0, w0, c0);
    sample_point_packed(p1, vmin, vmax, vx, vy, vz, brick_idx,
                        vertex_sigma, vertex_color, vertex_valid, meta,
                        sigma1, col1, w1, c1);
    sigma_m = 0.5f * (sigma0 + sigma1);
    col_m = make_float3(0.5f * (col0.x + col1.x),
...
__device__ inline void shade_segment_backward_packed(
    const float3& p0,
    const float3& p1,
    const float3& vmin,
    const float3& vmax,
    ...
    auto scatter = [&](const VertexWeight* ws, int cnt, float factor_sigma,
        ...
        for (int i = 0; i < cnt; ++i) {
            int id = ws[i].idx;
            float wgt = ws[i].w * factor_sigma;
            atomicAdd(grad_sigma + id, g_sigma_m * wgt);
            int base = id * 3;
            atomicAdd(grad_color + base + 0, fx * ws[i].w * 0.5f);
            atomicAdd(grad_color + base + 1, fy * ws[i].w * 0.5f);
            atomicAdd(grad_color + base + 2, fz * ws[i].w * 0.5f);
...
__global__ void rasterize_backward_kernel_packed(
    const float* __restrict__ rays_o,
    const float* __restrict__ rays_d,
    const int64_t* __restrict__ offsets,
    const int64_t* __restrict__ keys,
    ...
    BrickMeta meta{brick_starts, brick_shapes, brick_vertex_offsets};
    ...
    while (in_bounds(cx, cy, cz) && t0 <= t1) {
        ...
        int64_t off0 = offsets[b];
        int64_t off1 = offsets[b + 1];
        if (off1 > off0 && meta.shapes[b * 3 + 0] > 0) {
            float brick_exit_t = fminf(tMaxX, fminf(tMaxY, tMaxZ)) + eps;
            ...
            if (total_hits > kMaxHits) {
                return; // skip overly dense brick
            }
            ...
            for (int h = 0; h < hit_count; ++h) {
                ...
                float sigma_m, dt, exp_term;
                float3 col_m_dummy;
                compute_segment_values_packed(
                    p0_seg, p1_seg, vmin, vmax,
                    static_cast<int>(vx), static_cast<int>(vy), static_cast<int>(vz), static_cast<int>(b),
                    vertex_sigma, vertex_color, vertex_valid, meta,
                    sigma_m, col_m_dummy, dt, exp_term);
                hit_buf[h].sigma_m = sigma_m;
                hit_buf[h].dt = dt;
...
std::vector<torch::Tensor> rasterize_backward_packed_cuda(
    ...
    const int threads = 256;
    const int blocks = (static_cast<int>(num_rays) + threads - 1) / threads;
    rasterize_backward_kernel_packed<<<blocks, threads, 0, at::cuda::getDefaultCUDAStream()>>>(
        ro.data_ptr<float>(),
        rd.data_ptr<float>(),
        offsets.data_ptr<int64_t>(),
        keys.data_ptr<int64_t>(),
