...
struct GaussianKernal{
    ...
    double rotation[4];
    double SH[48];
    GaussianKernal() = default;
    GaussianKernal(double _x, double _y, double _z) : xyz{_x, _y, _z} {}
    GaussianKernal(double _x, double _y, double _z, double _alpha) : xyz{_x, _y, _z}, alpha(_alpha) {}

    GaussianKernal(double _x, double _y, double _z, double _alpha, double* _scale, double* _rotation, double* _SH) 
        : xyz{_x, _y, _z}, alpha(_alpha) {
            for (int i = 0; i < 3; i++) scale[i] = _scale[i];
            for (int i = 0; i < 4; i++) rotation[i] = _rotation[i];
            for (int i = 0; i < 48; i++) SH[i] = _SH[i];
        }
};
...
public:
    ...
    void split() {
        ...
        for (int i = 0; i < 8; ++i) {
            ...
            children[i]->kernal = new GaussianKernal(childCenter.x, childCenter.y, childCenter.z,
                                                     this->kernal->alpha, this->kernal->scale / 2.0,
                                                     this->kernal->rotation, this->kernal->SH);
        }
    }
